...
export default {
    ...
    data() {
        return {
            ...
            data: [
                { id: 1, title: 'Lorem Ipsum', code_template: "-", code_inspection: "-", assignment: 'Lorem ipsum',fleet: 'Lorem ipsum', period: 'Harian', date_assignment: '01 Juni 2024', status: 'completed'},
                { id: 2, title: 'Lorem Ipsum', code_template: "INS001", code_inspection: "INS001", assignment: 'Lorem ipsum',fleet: 'Lorem ipsum', period: 'Harian', date_assignment: '01 Juni 2024', status: 'pending'}
            ],
            params: {
                page: 1,
                limit: 10,
                        ...
                        <div class="live-preview">
                            <table-component :headers="headers" :data="data" :action="action" v-if="data.length > 0" @sort="sort($event.sortBy)">
                                ...
                                <template #status="{ item }">   
                                    <span :class="item.status === 'completed' ? 'badge rounded-pill bg-success-subtle text-success fs-12' : 'badge rounded-pill bg-danger-subtle text-danger fs-12'">{{ (item.status === 'completed') ? 'Sudah Dikerjakan' : 'Tidak Dikerjakan' }}</span>
                                </template>
                                <template #action="{ item }">
                                    <BButton v-if="item.status === 'completed'" variant="link" class="link-opacity-75 fs-22" size="sm" :to="`/inspection-response/view/${item.id}`">
                                        <img src="@/assets/icons/view.svg" alt="eye" />
                                    </BButton>
                                    <BButton variant="link" class="link-dark fs-22" size="sm" :to="`/inspection-response/edit/${item.id}`">
